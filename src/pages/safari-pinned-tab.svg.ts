/**
 * /safari-pinned-tab.svg — Safari's pinned-tab mask.
 *
 * Deliberately monochrome, and deliberately NOT recoloured here: Safari treats
 * this file as a silhouette and takes the colour from the `color` attribute on
 * the `<link rel="mask-icon">` tag, which BaseLayout sets from the palette. A
 * multi-colour file would be flattened and the second colour lost, so the
 * "dynamic" half lives in the link tag rather than in the asset.
 *
 * Production served this file on all three origins; the shape is unchanged.
 */
...
export const GET: APIRoute = () =>
  ...
  <g fill="#000000" transform="translate(0,700) scale(0.1,-0.1)">
