/**
 * /favicon.svg — the mark, in this palette, responding to the viewer's theme.
 *
 * The legacy favicons are a raster set generated years ago with the colours
 * baked in. They still ship (a browser that cannot read an SVG icon falls back
 * to /favicon.ico), but this is the one modern browsers prefer, and it is the
 * one that can change.
 *
 * "Dynamic" here means two real things, not an animation:
 *
 *   1. It is drawn from the design tokens rather than from whatever the old
 *      generator produced — sky `--c-maar` on the dark surface `--c-dark`.
 *   2. It honours `prefers-color-scheme`. A tab strip in light mode gets the
 *      mark in ink on paper; in dark mode, sky on near-black. A favicon with one
 *      baked background is invisible against half the browsers in the world.
 *
 * The values are inlined because an SVG served as its own file has no access to
 * the site's stylesheet — there is no cascade to inherit from. They are copied
 * from src/styles/tokens.css and that is the one place they may diverge, so
 * they are named here explicitly rather than left to look incidental.
 */
import type { APIRoute } from 'astro';

/** From src/styles/tokens.css. Keep in step by hand — see the note above. */
const SKY = '#a9d5e8';
const INK = '#100f14';
const PAPER = '#efe7da';
...
export const GET: APIRoute = () =>
  ...
  <style>
    /* Light is the default because a browser that does not report a preference
       is usually showing light chrome. */
    .field { fill: ${PAPER} }
    .mark  { fill: ${INK} }
  ...
  <rect class="field" width="700" height="700" rx="0"/>
  <g class="mark" transform="translate(0,700) scale(0.1,-0.1)">
